...
export default function MenuSantaBirria() {
  ...
  const updatePrice = (categoryId, itemId, value) => {
    setData(prev => prev.map(cat => {
      ...
      return { ...cat, items: cat.items.map(i => i.id === itemId ? { ...i, price: value } : i) };
    }));
  };
  ...
  const updateText = (categoryId, itemId, key, value) => {
    setData(prev => prev.map(cat => {
      if (cat.id !== categoryId) return cat;
      return { ...cat, items: cat.items.map(i => i.id === itemId ? { ...i, [key]: value } : i) };
    }));
  };
      ...
      <section className="relative" id="menu">
        <div className="mx-auto max-w-7xl px-4 pt-10 pb-4">
          <div className="flex flex-col md:flex-row items-center gap-6">
            <img src="/brand/logo.png" alt="La Santa Birria" className="w-24 h-24 object-contain" onError={(e)=>{e.currentTarget.style.display='none';}}/>
            <div>
              <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight">Menú – La Santa Birria</h1>
              <div className="mt-4 flex flex-wrap gap-2">
                <label className="flex items-center gap-2 rounded-full border px-3 py-1.5 bg-white shadow-sm">
                  <Search className="w-4 h-4"/>
                  <input className="outline-none text-sm" placeholder="Buscar plato o etiqueta..." value={query} onChange={(e)=>setQuery(e.target.value)} />
                </label>
                ...
                <button onClick={exportJSON} className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm bg-white border shadow-sm">
            ...
            <div className="mt-5 grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {cat.items.map(item => (
                <article key={item.id} className="bg-white rounded-2xl shadow-sm border overflow-hidden flex flex-col">
                  <div className="aspect-[4/3] bg-neutral-100">{item.img && <img src={item.img} alt={item.name} className="w-full h-full object-cover"/>}</div>
